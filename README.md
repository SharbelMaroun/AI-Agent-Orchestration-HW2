...
Default config uses `gemini-2.5-flash` for Dogs/Cats and `gemini-2.5-pro` for the Judge (`config/setup.json.models`). Budget cap = $5.00 (`budget_usd`); the gatekeeper logs a WARNING at 80% and raises `BudgetExceededError` at 100%.
